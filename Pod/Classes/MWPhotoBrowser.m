...
//    UIView * galleryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"gallery_icon"]];
//    galleryView.frame = CGRectMake(0, 0, 22, 30);
//    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(galleryButtonAction)];
//    [galleryView addGestureRecognizer:tap];
//    UIBarButtonItem * gallery = [[UIBarButtonItem alloc] initWithCustomView:galleryView];
//    [items addObject:gallery];
    ...
    [items addObject:cameraViewgallery];
    
    [items addObject:space];

    UIView * deleteView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"delete_icon"]];
    deleteView.frame = CGRectMake(0, 0, 22, 30);
    UITapGestureRecognizer * deleteViewtap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(deleteButtonAction)];
    [deleteView addGestureRecognizer:deleteViewtap];
    UIBarButtonItem * deleteViewgallery = [[UIBarButtonItem alloc] initWithCustomView:deleteView];
    [items addObject:deleteViewgallery];
    
    self.starImage = [UIImage imageNamed:@"star_icon"];
    UIView * starView = [[UIImageView alloc] initWithImage:self.starImage];
    starView.frame = CGRectMake(0, 0, 22, 22);
    UITapGestureRecognizer * starViewtap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(favoriteButtonAction)];
    [starView addGestureRecognizer:starViewtap];
    UIBarButtonItem * starViewgallery = [[UIBarButtonItem alloc] initWithCustomView:starView];
    self.navigationItem.rightBarButtonItem = starViewgallery;
...
- (void) galleryButtonAction {
    [_delegate galleryButtonDidTaped];
}

- (void) cameraButtonAction {
    [_delegate cameraButtonDidTaped];
}

- (void) deleteButtonAction {
    [_delegate deleteButtonDidTaped:self.currentIndex];
}

- (void) favoriteButtonAction {
    if (self.likedPhoto.intValue == self.currentIndex) {
        self.likedPhoto = [[NSNumber alloc] initWithInt:INT_MAX];
        [self changeFavoriteButton: [UIImage imageNamed:@"star_icon"]];
    } else {
        self.likedPhoto = [[NSNumber alloc] initWithInt: self.currentIndex];
        [self changeFavoriteButton: [UIImage imageNamed:@"star_favorite_icon"]];
    }
    [_delegate favoriteButtonDidTaped:self.currentIndex];
}

- (void) changeFavoriteButton: (UIImage*) iconImage {
    if (self.navigationItem.rightBarButtonItem != nil) {
        UIView * starView = [[UIImageView alloc] initWithImage:iconImage];
        starView.frame = CGRectMake(0, 0, 22, 22);
        UITapGestureRecognizer * starViewtap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(favoriteButtonAction)];
        [starView addGestureRecognizer:starViewtap];
        UIBarButtonItem * starViewgallery = [[UIBarButtonItem alloc] initWithCustomView:starView];
...
- (MWZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    ...
    self.navigationItem.rightBarButtonItem.image = [UIImage imageNamed:@"delete_icon"];
    return thePage;
}
...
- (void)configurePage:(MWZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    if (self.likedPhoto != nil) {
        if ([self.likedPhoto integerValue] == index) {
            [self changeFavoriteButton:[UIImage imageNamed:@"star_favorite_icon"]];
        } else {
            [self changeFavoriteButton:[UIImage imageNamed:@"star_icon"]];
        }
